...
mod tests {
    ...
    fn test_error_status_codes() {
        assert_eq!(
            PtcError::DockerNotAvailable("test".to_string()).status_code(),
            503
        );
        assert_eq!(
            PtcError::SessionNotFound("test".to_string()).status_code(),
            404
        );
        assert_eq!(
            PtcError::SessionExpired("test".to_string()).status_code(),
            410
        );
        assert_eq!(
            PtcError::InvalidToolResult("test".to_string()).status_code(),
            400
        );
