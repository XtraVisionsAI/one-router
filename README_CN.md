...
```
src/
├── api/                 # HTTP 处理器（messages、chat_completions、embeddings、rerank、models、health）
├── config/              # 配置与 AWS 设置
...
├── error/               # 错误类型
├── middleware/           # 认证与限流中间件
├── schemas/             # 请求/响应 Schema（Anthropic、OpenAI、Bedrock、Gemini、Embeddings、Rerank）
├── server/              # 应用启动、路由、状态管理
├── services/            # 业务逻辑
│   ├── backend_pool/    # 凭证池与负载均衡
│   ├── ptc/             # Programmatic Tool Calling（沙箱执行）
│   ├── bedrock.rs       # AWS Bedrock 服务（Converse + InvokeModel）
