...
async function save() {
  if (!form.value.source_model_id.trim()) { message.error('Source model ID is required'); return }
  if (!form.value.target_model_id.trim()) { message.error('Target model ID is required'); return }
