<script setup lang="ts">
  ...
  async function save() {
    ...
    if (!isEdit.value && !configJson.value.trim()) {
      message.error('Config JSON is required')
      return
    }
    if (configJson.value.trim() && configStatus.value === 'invalid') {
      message.error('Invalid JSON in config field')
      return
    }
    ...
    <NFormItem :label="isEdit ? 'Config' : 'Config JSON'" :required="!isEdit">
      <template v-if="isEdit">
        <div class="w-full">
          ...
          <n-code v-if="!showDecrypted" :code="configSummaryText" language="json" class="text-xs" />
          <NInput
            v-else
            :value="configJson"
            type="textarea"
          ...
          <div v-else-if="configStatus === 'valid' && showDecrypted" class="text-xs text-green-400 mt-1"
            >✓ Valid JSON</div
          >
